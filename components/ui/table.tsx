...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div data-slot="table-container" className="table-scroll relative">
      <table
        data-slot="table"
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b border-white/8 transition-colors hover:bg-white/[0.025] data-[state=selected]:bg-white/5",
        className,
      )}
      {...props}
    />
