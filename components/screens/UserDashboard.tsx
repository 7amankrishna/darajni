...
import { ProfileInput, Review, ReviewStatus } from "@/types";
...
export default function UserDashboard() {
  ...
  const [details, setDetails] = useState<ProfileInput>(emptyProfile);
  ...
  const setProfileField = <K extends keyof ProfileInput>(key: K, value: ProfileInput[K]) => {
  ...
  const saveProfile = async (event: FormEvent) => {
    ...
    setMessage(error || "Your details were updated.");
  };
  ...
  return (
    <main className="min-h-[70vh] py-12 sm:py-16">
      ...
      <div className="section-shell">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-3 text-sm text-white/43">
              Keep your contact details current and follow every review decision.
            </p>
          </div>
        ...
        {profile && profile.accountStatus !== "active" && (
          ...
          >
            <p className="text-xs font-bold uppercase tracking-wider text-white/65">
              {profile.accountStatus === "warned"
                ? "Private warning from DARAJNI"
                : "Your review activity is disabled"}
            </p>
        ...
        <section className="mt-8 glass-panel p-5 sm:p-7">
          <div>
            <p className="eyebrow">Basic details</p>
            <h2 className="font-display mt-2 text-3xl">Your profile</h2>
            <p className="mt-2 text-xs leading-6 text-white/38">
              These details help DARAJNI contact you and understand delivery requirements.
            </p>
          </div>
          ...
          <form onSubmit={saveProfile} className="mt-6 grid gap-5 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <label htmlFor="profile-address-1" className="field-label">Address line 1</label>
              <input
            ...
            <div className="sm:col-span-2">
              <label htmlFor="profile-address-2" className="field-label">Address line 2</label>
              <input
            ...
            <div>
              <label htmlFor="profile-state" className="field-label">State</label>
              <input
                id="profile-state"
                value={details.state}
            ...
            <div>
              <label htmlFor="profile-postal-code" className="field-label">Postal code</label>
              <input
            ...
            <div>
              ...
              <div className="field flex items-center text-white/45">{profile?.email}</div>
            </div>
            <div className="sm:col-span-2">
                  ...
                  <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                    <div>
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <span className={`status-pill ${reviewStatusClasses[review.status]}`}>
                          {review.status}
                        </span>
                      </div>
                    ...
                    <div className="flex shrink-0 gap-2">
                      {review.status !== "approved" && reviewsEnabled && (
                        <button type="button" onClick={() => beginEdit(review)} className="secondary-button">
                          Edit
                        </button>
                      )}
                      <button type="button" onClick={() => void removeReview(review)} className="danger-button">
                        Delete
                      </button>
                    </div>
                  </div>
      ...
      {editing && (
        <div className="fixed inset-0 z-[70] grid place-items-center overflow-y-auto bg-black/80 p-3">
          <form onSubmit={saveReview} className="glass-panel w-full max-w-lg p-6 sm:p-8">
